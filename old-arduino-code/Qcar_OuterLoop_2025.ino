...
const int LEFT_SERVO_ANGLE = 74;
const int STRAIGHT_SERVO_ANGLE = 92;
const unsigned long RIGHT_ACTION_DURATION = 3500;
const unsigned long LEFT_ACTION_DURATION = 4000;
const unsigned long STRAIGHT_ACTION_DURATION = 3400;
const float STOP_THRESHOLD = 1.60;
...
void performMovement(int servoAngle, unsigned long duration) {
  ...
  snprintf(debug_buffer, sizeof(debug_buffer), "Movement finished. Current state: IDLE");
  debug_msg.data = debug_buffer;
  debug_pub.publish(&debug_msg);
}
...
void messageCb1(const geometry_msgs::PointStamped& pose_msg) {
  // Only process when in IDLE or STOPPED
...
void messageCb(const std_msgs::Int32& angle_msg) {
  ...
  // Only apply steering if in IDLE state
...
void controlRobot() {
  if (currentState == IDLE || currentState == STOPPED) {
    if (y == 1.0 && x < STOP_THRESHOLD && x != 0.0) {
      ...
      // Tell the lane-keeping node to resume after stopping
      snprintf(mode_buffer, sizeof(mode_buffer), "auto_lane_keeping");
      mode_msg.data = mode_buffer;
      mode_pub.publish(&mode_msg);

      y = 5.0; // Reset y to ensure we don't re-trigger stop immediately
