...
const int FIRST_LEFT_SERVO_ANGLE = 74; //adjusted for sharper  turns 68
const int SECOND_LEFT_SERVO_ANGLE = 73;   // Adjusted for less sharp left turns73
const int STRAIGHT_SERVO_ANGLE = 95;  // Adjusted for better straight alignment (was 92)
const unsigned long FIRST_LEFT_ACTION_DURATION = 3500; // ms 3000
const unsigned long SECOND_LEFT_ACTION_DURATION = 4800;  // ms4800
const unsigned long STRAIGHT_ACTION_DURATION = 3500; // ms 2700
float STOP_THRESHOLD = 1.60; // Distance threshold for stopping 1.90
const int DRIVE_SPEED = 80; //was 72
    ...
    if (y == 1.0 && x < STOP_THRESHOLD && x != 0.0) {
      ...
      if (currentTime - lastLightDetectedTime > TRAFFIC_LIGHT_DELAY) {
        if (light == 1) {
          ...
          light = 2;
          lastLightDetectedTime = currentTime;
        } else if (light == 2) {
