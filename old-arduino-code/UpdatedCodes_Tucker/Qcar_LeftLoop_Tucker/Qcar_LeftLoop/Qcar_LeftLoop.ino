...
const int SECOND_LEFT_SERVO_ANGLE = 68; // Adjusted for less sharp left turns 65
const unsigned long FIRST_LEFT_ACTION_DURATION = 3600; // ms  
const unsigned long SECOND_LEFT_ACTION_DURATION = 3600; // ms 4000
...
int steering_angle = 90; // Default straight servo angle
    ...
    if (currentState == IDLE || currentState == STOPPED) {
        if (y == 1.0 && x < STOP_THRESHOLD && x != 0.0) {
            ...
            if (currentTime - lastLightDetectedTime > TRAFFIC_LIGHT_DELAY) {
                ...
                if (light == 1) {  // First green light: turn left
                    snprintf(debug_buffer, sizeof(debug_buffer), "Turning left at first green light, setting light to 2");
                    debug_msg.data = debug_buffer;
                    debug_pub.publish(&debug_msg);
                    
                    // Perform left turn at first traffic light
                    performMovement(FIRST_LEFT_SERVO_ANGLE, FIRST_LEFT_ACTION_DURATION);
                    
                    light = 2;  // Move to the second light
                    lastLightDetectedTime = currentTime;  // Update the time of detection
                } else if (light == 2) {  // Second green light: turn left
                    snprintf(debug_buffer, sizeof(debug_buffer), "Turning left at second green light, resetting light to 1");
                    debug_msg.data = debug_buffer;
                    debug_pub.publish(&debug_msg);
                    
                    // Perform left turn at second traffic light
                    performMovement(SECOND_LEFT_SERVO_ANGLE, SECOND_LEFT_ACTION_DURATION);
                    
                    light = 1;  // Reset to the first light for next cycle
                    lastLightDetectedTime = currentTime;  // Update the time of detection
                }
