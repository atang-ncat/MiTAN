...
const int LEFT_SERVO_ANGLE = 73; //73
const int STRAIGHT_SERVO_ANGLE = 80; //93
const unsigned long RIGHT_ACTION_DURATION = 100; // ms2200
const unsigned long LEFT_ACTION_DURATION = 50;  // ms 4800
const unsigned long STRAIGHT_ACTION_DURATION = 50; // ms 3500
const float STOP_THRESHOLD = 1.70; // Distance threshold for stopping 1.90
const int DRIVE_SPEED = 74;//75
...
void performMovement(int servoAngle, unsigned long duration) {
  ...
  //snprintf(debug_buffer, sizeof(debug_buffer), "Movement completed, returning to IDLE");
  //debug_msg.data = debug_buffer;
  //debug_pub.publish(&debug_msg);
...
void messageCb(const std_msgs::Int32& angle_msg) {
  ...
  //snprintf(debug_buffer, sizeof(debug_buffer), "Received steering angle: %d", steering_angle);
  //debug_msg.data = debug_buffer;
  //debug_pub.publish(&debug_msg);
...
void controlRobot() {
  if (currentState == IDLE || currentState == STOPPED) {
    //snprintf(debug_buffer, sizeof(debug_buffer), "Control loop entered. Current state: %d", currentState);
    //debug_msg.data = debug_buffer;
    //debug_pub.publish(&debug_msg);
    ...
    if (y == 1.0 && x < STOP_THRESHOLD && x != 0.0) {
      ...
      if (light == 1) {  // First green light: turn right
        ...
        light = 2;
      } else if (light == 2) {  // Second green light: go straight
        ...
        light = 3;
      } else if (light == 3) {  // Third green light: turn left
        snprintf(debug_buffer, sizeof(debug_buffer), "Turning left at green light, resetting light to 1");
        debug_msg.data = debug_buffer;
        debug_pub.publish(&debug_msg);

        if ( y == 3 && x < 1.80 ) {
          performMovement(LEFT_SERVO_ANGLE, LEFT_ACTION_DURATION);//74
        } else {
      ...
      drive(steering_angle, 70);  // Keep driving with current steering angle
